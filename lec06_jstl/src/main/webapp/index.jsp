...
<body>
	...
	<p>이룸 : ${planet.name}</p>
	<p>태양으로부터의 거리 : ${planet.distance} km</p>
	...
	<p>국어, 영어, 수학 점수 평균 => 나누기 : </p>
	<p>${kor + eng + math div 3 }</p>
	
	<p>평균 점수가 80점 초과 => "합격" 그렇지 않으면 "불합격"</p>
	<p>${kor + eng + math div 3 gt 80 ? "합격" : "불합격"}</p>
	
	<p>이순신김유신 처럼 이름 연결</p>
	${name1 }${name2 }
	
	<p>두분의 이름이 동일한지 비교 => "동일 인물" 그렇지 않으면 "다르다"</p>
	${name1 eq name2 ? "동일 인물" : "다르다" }
	
	<p>heroList가 비어있는지 판단 -> "O" 그렇지 않으면 "X"</p>
	${empty heroList ? "O" : "X" }
	
	<p>국어 점수가 영어 점수보다 높거나 heroList가 비어 있지 않은 경우 true 출력</p>
	${kor gt eng or not empty heroList}
	
	<p>수학 점수가 80점 미만 이고, 영어 점수가 90점 이상이면 true 출력 --%></p>
	${math lt 80 and eng ge 90}
	
	<%-- 논리연산과 비교연산을의 결과를 삼항연산자로 표현하고 싶은경우 --%>
	<%-- (괄호안에 연산식) ? true값 : false값 --%>
	...
	<c:choose>
		...
		<c:when test="${num3 ge 10}">
			<p>num3 20보다 작거나 같으면서, 10보다 크거나 같습니까?</p>
		</c:when>
		<c:otherwise>
	...
	<!-- 포맷팅은 그 자체가 expression이기 때문에 출력됨 -->
	<h2>(1) 숫자 정보 포맷팅</h2>
	...
	<form action="/printStar" method="get">
		<label for="star_num">별의 갯수(숫자입력)</label>
		<input type="number" name="star_num" id="star_num">
		<input type="submit" name="btn_star" value="출력하기">
	</form>
